 ...
 * @returns The Channels available on the CCU
 */
export async function getChannels(host: string, authentication?: { user: string, password: string }): Promise<Channel[]> {
    ...
    for (const channel of rawChannels) {
        ...
        if (channel.iface === '1009') {
            channel.iface = INTERFACE_TYPE.BIDCOSRF;
        } else if (channel.iface === '1011') {
            channel.iface = INTERFACE_TYPE.HMIP;
        } else {
            channel.iface = undefined;
        }
    }

    return z.array(Channel).parse(rawChannels);
}
